...
app.post('/twiml', (req, res) => {
  const { sessionId, to, leadName, whisper } = req.query;
  ...
  twiml.say(`Connected lead: ${leadName || 'Unknown Lead'}. Press 1 to claim this call.`);  twiml.gather({
    numDigits: 1,
    action: `/claim?sessionId=${encodeURIComponent(sessionId)}&to=${encodeURIComponent(to || '')}`,    method: 'POST',
    timeout: 10
  });
...
app.post('/claim', async (req, res) => {
  ...
  await addNoteToHighLevel(session.contactId, `Parallel dial answered by ${to || 'unknown number'}.`);
